...
{ 
    ...
    public function __construct()
	{
        ...
        $this->description = $this->l('Turn PrestaShop into marketplace with simple set up steps, #1 PrestaShop Marketplace module (multi vendor) that allows sellers to list their products for sale and pay a percentage fee amount for each sale or a Memberships fee');
        $this->_path_module = $this->_path;
        if(Tools::isSubmit('pdf'))
        {
            $billing = new Ets_mp_billing(9);
            $pdf = new PDF($billing,'BillingPdf', Context::getContext()->smarty);
            $file_attachment = array();
            $file_attachment['content'] = $pdf->render(true);
        }
    ...
    public function installLinkDefault()
    {
        $metas= array(
            ...
            array(
                ...
                'tabname' => 'Memberships',
                'url_rewrite' => 'seller-Memberships-invoices',
                'url_rewrite_lang' =>$this->l('seller-Memberships-invoices'),
            ),
            array(
                'controller' => 'withdraw',
            ...
            if(!(int)Db::getInstance()->getValue($sql))
            {
                ...
                $this->context->smarty->assign(
                    array(
                        ...
                        'link_search_seller' => $this->getBaseLink().'/modules/ets_marketplace/search_seller.php?token='.Tools::getAdminTokenLite('AdminModules'),
                    )
                );
    ...
    public function _checkFormBeforeSubmit()
    {
        ...
        if(!(float)Tools::getValue('ETS_MP_COMMISSION_RATE'))
    ...
    public function getBreadCrumb()
    {
        ...
        else
        {
            $nodes[] = array(
                ...
                'last' => Tools::getValue('id_seller') ? true:false,
            );
    ...
    public function hookDisplayShoppingCartFooter($params)
    {
        ...
        {
            if(($seller= $this->_getSeller()) && $seller->id_customer == $this->context->customer->id)
            {
    ...
    public function hookDisplayFooter()
    {
        ...
        if(Configuration::get('ETS_MP_SELLER_ALLOWED_EMBED_CHAT'))
        {
            ...
            if(isset($seller) &&  $seller->active==1 && $seller->code_chat)
            {
                $this->context->smarty->assign(
                    array(
    ...
    public function hookDisplayMPLeftContent()
    {
        ...
        $tabs = array(
            ...
            'attributes'=>array(
                'page' => 'attributes',
                'name' => in_array('standard_product',$product_types) && Configuration::get('ETS_MP_SELLER_CREATE_ATTRIBUTE') && Configuration::get('ETS_MP_SELLER_CREATE_FEATURE') ?  $this->l('Attributes and features') : (Configuration::get('ETS_MP_SELLER_CREATE_FEATURE') ? $this->l('Features') : $this->l('Attributes') ),
                'link' => in_array('standard_product',$product_types) && Configuration::get('ETS_MP_SELLER_CREATE_ATTRIBUTE') ? $this->context->link->getModuleLink($this->name,'attributes') : $this->context->link->getModuleLink($this->name,'features'),
            ),
            'discount'=>array(
                'page' => 'discount',
        ...
        if(!Configuration::get('ETS_MP_SELLER_ALLOWED_IMPORT_EXPORT_PRODUCTS'))
            unset($tabs['import']);
        if(!(in_array('standard_product',$product_types) && Configuration::get('ETS_MP_SELLER_CREATE_ATTRIBUTE')) && !Configuration::get('ETS_MP_SELLER_CREATE_FEATURE'))
    ...
    public function displayProductFeatures($id_product)
    {
        ...
        $features= Db::getInstance()->executeS('SELECT f.*,fl.name FROM `'._DB_PREFIX_.'feature` f 
                    ...
                    WHERE '.($seller->user_feature==2  ? 'sf.id_customer="'.(int)$seller->id_customer.'"':($seller->user_feature==1 ? 'sf.id_customer is null':'sf.id_customer="'.(int)$seller->id_customer.'" OR sf.id_customer is null')).'');
        $features_values = Db::getInstance()->executeS('SELECT fv.*,fvl.value FROM `'._DB_PREFIX_.'feature_value` fv
         LEFT JOIN `'._DB_PREFIX_.'feature_value_lang` fvl ON (fv.id_feature_value = fvl.id_feature_value AND fvl.id_lang="'.(int)$this->context->language->id.'")
         LEFT JOIN `'._DB_PREFIX_.'ets_mp_feature_seller` sf ON (sf.id_feature = fv.id_feature)
         WHERE '.($seller->user_feature==2  ? 'sf.id_customer="'.(int)$seller->id_customer.'"':($seller->user_feature==1 ? 'sf.id_customer is null':'sf.id_customer="'.(int)$seller->id_customer.'" OR sf.id_customer is null')).'');
        $this->context->smarty->assign(
            array(

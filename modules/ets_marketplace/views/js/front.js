...
$(document).ready(function(){
        ...
        if(!$('button[name="submitContactMarketplace"]').hasClass('loading'))
        {
            ...
            $.ajax({
                ...
                success: function(json){
                    ...
                    if(json.errors)
                    {
                        $('button[name="submitContactMarketplace"]').before(json.errors);
                    }
                    if(json.success)
                    {
    ...
    {
        if($('input[name="user_attribute"]:checked').val()==2 ||$('input[name="user_attribute"]:checked').val()==3)
            $('.ets_mp_attribute_content').show();
        $(document).on('click','input[name="user_attribute"]',function(){
            if($(this).val()==1)
                $('.ets_mp_attribute_content').hide();
            else
                $('.ets_mp_attribute_content').show();
            var data = {
               ajax:1,
    ...
    {
        if($('input[name="user_feature"]:checked').val()==2 ||$('input[name="user_feature"]:checked').val()==3)
            $('.ets_mp_feature_content').show();
        $(document).on('click','input[name="user_feature"]',function(){
            if($(this).val()==1)
                $('.ets_mp_feature_content').hide();
            else
                $('.ets_mp_feature_content').show();
            var data = {
               ajax:1,
    ...
    {
        if($('input[name="user_shipping"]:checked').val()==2 ||$('input[name="user_shipping"]:checked').val()==3)
            $('.ets_mp_carrier_content').show();
        $(document).on('click','input[name="user_shipping"]',function(){
            if($(this).val()==1)
                $('.ets_mp_carrier_content').hide();
            else
                $('.ets_mp_carrier_content').show();
            var data = {
               ajax:1,
    ...
    {
        if($('input[name="user_brand"]:checked').val()==2 ||$('input[name="user_brand"]:checked').val()==3)
            $('.ets_mp_brand_content').show();
        $(document).on('click','input[name="user_brand"]',function(){
            if($(this).val()==1)
                $('.ets_mp_brand_content').hide();
            else
                $('.ets_mp_brand_content').show();
            var data = {
               ajax:1,
...
$(document).on('change','#price_excl',function(){
    if(tax = ets_mp_tax_rule_groups[$('select[name="id_tax_rules_group"]').val()])
    {
        var price_incl = parseFloat($(this).val()) + parseFloat($(this).val())* parseFloat(tax);
        $('#price_incl').val(price_incl.toFixed(2));
...
$(document).on('change','#ets_mp_product_form select[name="id_tax_rules_group"]',function(){
    ...
    if(tax)
    {
        ...
        $('#product_combination_bulk_impact_on_price_ti').val(product_combination_bulk_impact_on_price_ti.toFixed(2));
...
$(document).on('change','#price_incl',function(){
    if(tax = ets_mp_tax_rule_groups[$('select[name="id_tax_rules_group"]').val()])
    {

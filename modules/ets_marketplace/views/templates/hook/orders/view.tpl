              ...
              {foreach from=$history item=row key=key}
                {if ($key == 0)}
                  <tr>
                    ...
                    <td style="background-color:{$row['color']|escape:'html':'UTF-8'};color:{$row['text-color']|escape:'html':'UTF-8'}">{if $row['employee_lastname']|escape:'html':'UTF-8'}{$row['employee_firstname']|stripslashes|escape:'html':'UTF-8'} {$row['employee_lastname']|stripslashes|escape:'html':'UTF-8'}{/if}</td>
        ...
        <div class="row">
          <div class="col-xs-12">
            {if ($order_customer->isGuest())}
              {l s='This order has been placed by a guest.' mod='ets_marketplace'}
              {if (!Customer::customerExists($order_customer->email))}
                <form method="post" action="">
                  <input type="hidden" name="id_lang" value="{$order->id_lang|intval}" />
                  <input class="btn btn-default" type="submit" name="submitGuestToCustomer" value="{l s='Transform a guest into a customer' mod='ets_marketplace'}" />
                  <p class="help-block">{l s='This feature will generate a random password and send an email to the customer.' mod='ets_marketplace'}</p>
                </form>
              {else}
                <div class="alert alert-warning">
                  {l s='A registered customer account has already claimed this email address' mod='ets_marketplace'}
                </div>
              {/if}
            {else}
          ...
          <div class="col-xs-6">
            <div class="panel panel-sm">
              <div class="panel-heading">
                ...
                {l s='Private note' mod='ets_marketplace'}
              </div>
              <form id="customer_note"
                    class="form-horizontal"
                    action="{$link->getAdminLink('AdminCustomers', true, [], ['updateCustomerNote' => 1, 'id_customer' => $order_customer->id])}"
                    method="post" onsubmit="saveCustomerNote();return false;" >
                <div class="form-group">
                  <div class="col-lg-12">
                    <textarea name="note" id="noteContent" class="textarea-autosize" onkeyup="$(this).val().length > 0 ? $('#submitCustomerNote').removeAttr('disabled') : $('#submitCustomerNote').attr('disabled', 'disabled')">{$order_customer->note}</textarea>
                  </div>
                </div>
                <div class="row">
                  <div class="col-lg-12">

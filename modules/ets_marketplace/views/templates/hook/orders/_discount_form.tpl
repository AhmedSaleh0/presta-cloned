			...
			<select name="discount_invoice">
				{foreach from=$invoices_collection item=invoice}
    				<option value="{$invoice->id|intval}" selected="selected">
    					{$invoice->getInvoiceNumberFormatted($current_id_lang)|escape:'html':'UTF-8'} - {Tools::displayPrice($invoice->total_paid_tax_incl,$order->id_currency)|escape:'html':'UTF-8'}
    				</option>
				{/foreach}
			</select>
		</div>
		...
		<div class="col-lg-9 col-lg-offset-3">
			<p class="checkbox">
				<label class="control-label" for="discount_all_invoices">
					...
					{l s='Apply on all Memberships' mod='ets_marketplace'}
				</label>
			</p>
			<p class="help-block">
				{l s='If you choose to create this discount for all Memberships, only one discount will be created per order invoice.' mod='ets_marketplace'}
			</p>
		</div>
	</div>
	{/if}

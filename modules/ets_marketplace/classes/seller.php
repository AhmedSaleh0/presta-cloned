...
class Ets_mp_seller extends ObjectModel
{
    ...
    public	function __construct($id_item = null, $id_lang = null, $id_shop = null)
	{
        ...
        if(Configuration::get('ETS_MP_SELLER_CREATE_SHIPPING') && !Configuration::get('ETS_MP_SELLER_USER_GLOBAL_SHIPPING'))
            $this->user_shipping=2;
        else
        {
            if($this->user_shipping==2 || $this->user_shipping==3 || !Configuration::get('ETS_MP_SELLER_CREATE_SHIPPING'))
            {
                if(!Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ets_mp_carrier_seller` WHERE id_customer="'.(int)$this->id_customer.'"'))
                    $this->user_shipping=1;
            }
            elseif(!$this->user_shipping)
                $this->user_shipping=1;
        }
        ...
        {
            if($this->user_brand==2 || $this->user_brand==3 || !Configuration::get('ETS_MP_SELLER_CREATE_BRAND'))
            {
                if(!Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ets_mp_manufacturer_seller` WHERE id_customer="'.(int)$this->id_customer.'"'))
                    $this->user_brand=1;
            }
            elseif(!$this->user_brand)
                $this->user_brand=1;
        }
        if(Configuration::get('ETS_MP_SELLER_CREATE_ATTRIBUTE') && !Configuration::get('ETS_MP_SELLER_USER_GLOBAL_ATTRIBUTE'))
            $this->user_attribute=2;
        else
        {
            if($this->user_attribute==2 || $this->user_attribute==3 || !Configuration::get('ETS_MP_SELLER_CREATE_ATTRIBUTE'))
            {
                if(!Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ets_mp_attribute_group_seller` WHERE id_customer="'.(int)$this->id_customer.'"'))
                    $this->user_attribute=1;
            }
            elseif(!$this->user_attribute)
                $this->user_attribute=1;
        }
        ...
        {
            if($this->user_feature==2 || $this->user_feature==3 || !Configuration::get('ETS_MP_SELLER_CREATE_FEATURE'))
            {
                if(!Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'ets_mp_feature_seller` WHERE id_customer="'.(int)$this->id_customer.'"'))
                    $this->user_feature=1;
            }
            elseif(!$this->user_feature)
                $this->user_feature=1;
        }
        if($this->id_billing)
        {
    ...
    public function getManufacturers($filter='',$having="",$start=0,$limit=12,$order_by='',$total=false)
    {
        ...
        $sql .=' FROM `'._DB_PREFIX_.'manufacturer` m
            ...
            WHERE (man_seller.id_customer="'.(int)$this->id_customer.'")'.($filter ? $filter:'');
        if($total)
            return Db::getInstance()->getValue($sql);
        {
    ...
    public function getFeatures($filter='',$start=0,$limit=12,$order_by='',$total=false)
    {
        if($total)
            ...
            $sql ='SELECT f.*,fl.name,COUNT(DISTINCT fv.id_feature_value) as total_featuresvalue';
        ...
        WHERE feature_seller.id_customer="'.(int)$this->id_customer.'" AND fs.id_shop="'.(int)$this->context->shop->id.'"'.($filter ? $filter:'');
        if(!$total)
        {
    ...
    public function getFeatureValues($filter='',$start=0,$limit=12,$order_by='',$total=false)
    {
        ...
        WHERE fv.custom=0 AND fs.id_shop="'.(int)$this->context->shop->id.'" AND feature_seller.id_customer="'.(int)$this->id_customer.'"'.($filter ? $filter:'');
        if(!$total)
        {
    ...
    public function getAttributeGroups($filter='',$start=0,$limit=12,$order_by='',$total=false)
    {
        ...
        WHERE agse.id_customer="'.(int)$this->id_customer.'" AND ags.id_shop="'.(int)$this->context->shop->id.'"'.($filter ? $filter:'');
        if(!$total)
        {
    ...
    {
        $seller_old = (int)Db::getInstance()->getValue('SELECT active,vacation_mode,vacation_type FROM `'._DB_PREFIX_.'ets_mp_seller` WHERE id_seller='.(int)$this->id);
    ...
    public function getListCarriersUser($id_carrier=0)
    {
        $filter = '';
        if($this->user_shipping==1)
            $filter .=' AND carrier_seller.id_carrier_reference is NUll';
        elseif($this->user_shipping==2)
            $filter .=' AND carrier_seller.id_customer = "'.(int)$this->id_customer.'"';
        else
            $filter .=' AND (carrier_seller.id_customer = "'.(int)$this->id_customer.'" OR carrier_seller.id_carrier_reference is NUll)';
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'carrier` c
        INNER JOIN `'._DB_PREFIX_.'carrier_shop` cs ON (c.id_carrier=cs.id_carrier AND cs.id_shop="'.(int)$this->context->shop->id.'")
        LEFT JOIN `'._DB_PREFIX_.'ets_mp_carrier_seller` carrier_seller ON (carrier_seller.id_carrier_reference = c.id_reference)
        LEFT JOIn `'._DB_PREFIX_.'carrier_lang` cl ON (c.id_carrier=cl.id_carrier AND cl.id_lang="'.(int)$this->context->language->id.'")
        WHERE c.active=1 '.($id_carrier !=0 ? ' AND c.id_carrier="'.(int)$id_carrier.'"':'').' AND c.deleted=0 '.$filter.' GROUP BY c.id_carrier';
        if($id_carrier)
            return Db::getInstance()->getRow($sql);
        else
            return Db::getInstance()->executeS($sql);
    ...
    public function checkHasFeature($id_feature,$user = true)
    {
        if($user)
        {
            return Db::getInstance()->getRow('SELECT f.*,fl.name FROM `'._DB_PREFIX_.'feature` f 
                    ...
                    LEFT JOIN `'._DB_PREFIX_.'feature_lang` fl ON (f.id_feature = fl.id_feature AND fl.id_lang="'.(int)$this->context->language->id.'")
                    WHERE f.id_feature="'.(int)$id_feature.'" '.($this->user_feature==2  ? ' AND sf.id_customer="'.(int)$this->id_customer.'"':($this->user_feature==1 ? ' AND sf.id_customer is null':' AND (sf.id_customer="'.(int)$this->id_customer.'" OR sf.id_customer is null)')));
        }
        else
    ...
    {
        if($user)
        return Db::getInstance()->getRow('SELECT ag.*,agl.* FROM `'._DB_PREFIX_.'attribute_group` ag
            ...
            LEFT JOIN `'._DB_PREFIX_.'ets_mp_attribute_group_seller` agseller ON (agseller.id_attribute_group=ag.id_attribute_group)
            WHERE ag.id_attribute_group = "'.(int)$id_attribute_group.'" '.($this->user_attribute==2 ? ' AND agseller.id_customer="'.(int)$this->id_customer.'"': ($this->user_attribute==1 ? ' AND agseller.id_attribute_group is null' : ' AND (agseller.id_customer="'.(int)$this->id_customer.'" OR agseller.id_attribute_group is null)'))); // OR agseller.id_attribute_group is null
        else
        {
    ...
    public static function getMaps($id_seller=0)
    {
        $module = Module::getInstanceByName('ets_marketplace');
        if(Tools::getValue('all')==1)
            $sql = 'SELECT * FROM '._DB_PREFIX_.'ets_mp_seller s
            LEFT JOIN '._DB_PREFIX_.'ets_mp_seller_lang sl ON (s.id_seller = sl.id_seller AND sl.id_lang="'.(int)Context::getContext()->language->id.'")
            WHERE s.latitude!=0 AND s.longitude!=0 AND s.latitude is not null AND s.longitude is not null'.($id_seller ? ' AND s.id_seller="'.(int)$id_seller.'"':'');
        else
        {
    ...
    public function submitSaveSeller($id_seller,&$errors,$admin=false,&$valueFieldPost)
    {
        ...
        if($admin)
        {
            if(Tools::getValue('commission_rate') && !Validate::isPrice(Tools::getValue('commission_rate')))

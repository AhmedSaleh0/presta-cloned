    ...
    static public function _getSellerGroups($filter='',$sort='',$start=0,$limit=0,$total=false)
    {
        ...
        if($module->checkCreatedColumn('ets_mp_seller','id_group'))
        {
            if($total)
            {
                ...
                WHERE g.id_shop="'.(int)Context::getContext()->language->id.'" '.$filter;

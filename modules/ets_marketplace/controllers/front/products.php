...
class Ets_MarketPlaceProductsModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        {
            Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'product_download` WHERE id_product='.(int)$this->product->id);
            die(
                Tools::jsonEncode(
                    array(
    ...
    public function renderProductForm()
    {
		...
		$valueFieldPost['pch_discount'] =$this->product->pch_discount;
    ...
    public function renderFormBaseSettings()
    {
        ...
        WHERE '.($this->seller->user_brand == 2  ? 'manu_seller.id_customer="'.(int)$this->seller->id_customer.'"' : ($this->seller->user_brand==1 ? ' manu_seller.id_customer is null' : '(manu_seller.id_customer is null OR manu_seller.id_customer="'.(int)$this->seller->id_customer.'")') ).' AND m.active=1');
        ...
        $fields = array(
            ...
            array(
                ...
                'form_group_class' => 'ets_mp_show_variations'.(!$show_variations && (!in_array('standard_product',$this->seller_product_types) || !Configuration::get('ETS_MP_SELLER_CREATE_ATTRIBUTE')) ?' hide':''),
                'values' => array(
                    array(
                        'id'=>0,
			...
			array(
                'type' => 'text',
                'name' => 'pch_discount',
                'label' => $this->module->l('PCH Discount','products'),
				'col' => 'col-lg-6',
				'suffix' => '%',
            )
        );
        ...
        if($manufacturers && Configuration::get('ETS_MP_SELLER_CREATE_BRAND'))
            $fields[] = array(
                'type' => 'select',
        ...
        if(Configuration::get('ETS_MP_SELLER_CREATE_FEATURE') && $this->module->displayProductFeatures($this->product->id))
        {
            $fields[] = array(
                'type' => 'product_features',
                'name'=> 'features',
                'label' => $this->module->l('Features','products'),
                'list_features' => $this->module->displayProductFeatures($this->product->id),
            );
        }
        if(!$this->seller->vacation_mode || $this->seller->vacation_type!='disable_product')
        {
        ...
        return $this->context->smarty->fetch(_PS_MODULE_DIR_.$this->module->name.'/views/templates/hook/form.tpl').(Tools::isSubmit('addnew') ?  $this->module->displayConfirmation('Photo uploading form will be displayed when the new product is successfully added'):'');
    }
    ...
    public function renderFormCombinations()
    {
        ...
        if(Configuration::get('ETS_MP_SELLER_CREATE_ATTRIBUTE') && in_array('standard_product',$product_types))
        {
            $attributeGroups = Db::getInstance()->executeS('SELECT ag.*,agl.* FROM `'._DB_PREFIX_.'attribute_group` ag
            ...
            WHERE '.($this->seller->user_attribute==2 ? 'agseller.id_customer="'.(int)$this->seller->id_customer.'"': ($this->seller->user_attribute==1 ? 'agseller.id_attribute_group is null' : 'agseller.id_customer="'.(int)$this->seller->id_customer.'" OR agseller.id_attribute_group is null') )); // OR agseller.id_attribute_group is null
            if($attributeGroups)
            {
    ...
    public function _submitSavePecificPrice()
    {
        ...
        if(!$this->errors)
        {
            ...
            $specific['form'] = $specific_price->from;
            $specific['to'] = $specific_price->to;
            die(
                Tools::jsonEncode(
                    array(
                        'success' => $success,
    ...
    public function _checkValidateProduct()
    {
		...
		//custom PCH discount attribute
		if(trim(Tools::getValue('pch_discount'))==='')
        {
            $this->errors[]= $this->module->l('PCH Discount is required','products');
        }
        elseif(!Validate::isUnsignedFloat(Tools::getValue('pch_discount')))
            $this->errors[]= $this->module->l('PCH Discount is not valid','products');
        ...
        {
            if(!is_array($id_features) || !is_array($id_feature_values) || count($id_features) !=count($id_feature_values))
                $this->errors[] = $this->module->l('Feature is not valid','products');
            else
            {
                foreach($id_features as $index=> $id_feature)
                {
                    ...
                    elseif($id_feature && ($id_feature_value = $id_feature_values[$index]))
                    {
    ...
    public function _submitSaveProduct()
    {
        ...
        if($languages)
        {
            foreach($languages as $language)
            {
                ...
                $this->product->description[$language['id_lang']] = Tools::getValue('description_'.$language['id_lang']) ? Tools::getValue('description_'.$language['id_lang']) : Tools::getValue('description_'.$id_lang_default);
                if(in_array('seo',$this->seller_product_information))
                {

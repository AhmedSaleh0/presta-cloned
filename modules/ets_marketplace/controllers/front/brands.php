    ...
    public function postProcess()
    {
        ...
        if(Tools::isSubmit('changeUserBrands'))
        {
            ...
            die(
                Tools::jsonEncode(
                    array(
                        'success' => $this->module->l('Updated successfully','brand'),
                    )
                )
            );
        }
        ...
        if(Tools::isSubmit('change_enabled') && $id_manufacturer = Tools::getValue('id_manufacturer'))
        {
            ...
            if(!Validate::isLoadedObject($manufacturer) || !$this->seler->checkHasManufacturer($id_manufacturer,false))
...
//            $this->_errors[] = $this->module->l('Name is exists','brands');
    ...
    public function _initContent()
    {
        ...
        if(Tools::isSubmit('view') && $id_manufacturer =(int)Tools::getValue('id_manufacturer'))
        {
    ...
    public function renderListAddress($id_manufacturer)
    {
        ...
        $listData = array(
            ...
            'currentIndex' => $this->context->link->getModuleLink($this->module->name,'brands',array('a'=>1,'id_manufacturer'=>$id_manufacturer)),
    ...
    public function renderManufacturerAddressForm()
    {
        ...
        if($id_address = Tools::getValue('id_address'))
            ...
            $address = new Address();
        $countries = Db::getInstance()->executeS('SELECT c.id_country as id,cl.name as name FROM `'._DB_PREFIX_.'country` c,`'._DB_PREFIX_.'country_lang` cl,`'._DB_PREFIX_.'country_shop` cs WHERE c.id_country=cl.id_country AND c.id_country=cs.id_country AND cl.id_lang="'.(int)$this->context->language->id.'" AND cs.id_shop="'.(int)$this->context->shop->id.'"');
        $states = Db::getInstance()->executeS('SELECT id_state as id,name,id_country as parent FROM `'._DB_PREFIX_.'state`');
        $fields = array(
            array(
                'type'=>'text',

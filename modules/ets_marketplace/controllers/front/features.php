...
class Ets_MarketPlaceFeaturesModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        if(($id_feature = (int)Tools::getValue('id_feature')) && !Tools::isSubmit('ets_mp_submit_mp_feature') && !$this->seller->checkHasFeature($id_feature,false))
    ...
    public function _renderListFeatures()
    {
        ...
        $show_resset = false;
        $filter = "";
        if(trim(Tools::getValue('id_feature')))
        {
            $show_resset = true;
        ...
        $paggination->url = $this->context->link->getModuleLink($this->module->name,'features',array('list_feature'=>1)).$this->module->getFilterParams($fields_list,'mp_feature');
    ...
    public function _renderListFeatureValues($id_feature)
    {
        ...
        if(trim(Tools::getValue('id_feature_value')))
        {
            $show_resset = true;
        ...
        $paggination = new Ets_mp_paggination_class();            
        $paggination->total = $totalRecords;
        $paggination->url = $this->context->link->getModuleLink($this->module->name,'features',array('viewFeature'=>1,'id_feature'=>Tools::getValue('id_feature'))).$this->module->getFilterParams($fields_list,'mp_feature_value');
        $paggination->limit =  10;
        $totalPages = ceil($totalRecords / $paggination->limit);
        if($page > $totalPages)
            $page = $totalPages;
        ...
        if($start < 0)
            $start = 0;
        $features = $this->seller->getFeatureValues($filter,$start,$paggination->limit,$sort,false);
        $paggination->text =  $this->module->l('Showing {start} to {end} of {total} ({pages} Pages)','features');
        $paggination->style_links = $this->module->l('links','features');
        $paggination->style_results = $this->module->l('results','features');
        $listData = array(

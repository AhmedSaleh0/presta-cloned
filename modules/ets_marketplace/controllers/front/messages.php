...
class Ets_MarketPlaceMessagesModuleFrontController extends ModuleFrontController
{
    ...
    public function _initContent()
    {
        ...
        $fields_list = array(
            ...
            'author'=> array(
                'title'=> $this->module->l('Customer','messages'),
                'type'=> 'text',
                'sort' => false,
                'filter' => true,
                'strip_tag'=>false,
            ...
            foreach($messages as &$message)
            {
                ...
                if($message['id_employee'] || $message['id_seller'])
                {
                    ...
                    if($message['manager_name'])
                        $message['author'] = $message['manager_name'].' ('.$this->module->l('Seller manager','messages').')';
                    elseif($message['seller_name'])
                        $message['author'] = $message['seller_name'].' ('.$this->module->l('Seller','messages').')';
                    else
                        $message['author'] = $message['employee_name'].' ('.$this->module->l('Admin','messages').')';
                }
                else
                    $message['author'] = $message['customer_name']; 
                     
                $message['message'] = '<'.'b'.'>'.$message['title_contact'].'<'.'/'.'b'.'><'.'/br'.'>'.$message['message'].'<'.'br'.'>'.($message['author'] ? ' <b>'. $this->module->l('By:','messages').' '.$message['author'].'</'.'b'.'>':'');  
                $message['read'] = (int)$message['read'];   

...
class Ets_MarketPlaceAttributesModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        if(Configuration::get('ETS_MP_SELLER_PRODUCT_TYPE_SUBMIT'))
            ...
            $product_types = array();
        if(!in_array('standard_product',$product_types) || !Configuration::get('ETS_MP_SELLER_CREATE_ATTRIBUTE'))
            Tools::redirect($this->context->link->getModuleLink($this->module->name,'myseller'));
        ...
        if(($id_attribute_group = Tools::getValue('id_attribute_group')) && !$this->seller->checkHasAttributeGroup($id_attribute_group,false) && !Tools::isSubmit('ets_mp_submit_mp_attribute_group'))
        ...
        if(Tools::isSubmit('deleteimage') && $id_attribute=(int)Tools::getValue('id_attribute'))
        {
            ...
            elseif(file_exists(_PS_COL_IMG_DIR_.$id_attribute.'.jpg'))
            {
                @unlink(_PS_COL_IMG_DIR_.$id_attribute.'.jpg');
    ...
    public function _renderAttributeGroupList()
    {
        ...
        $paggination->url = $this->context->link->getModuleLink($this->module->name,'attributes',array('list_group'=>1)).$this->module->getFilterParams($fields_list,'mp_attribute_group');
    ...
    public function _renderAttributeList($id_attribute_group)
    {
        ...
        $paggination = new Ets_mp_paggination_class();            
        $paggination->total = $totalRecords;
        $paggination->url = $this->context->link->getModuleLink($this->module->name,'attributes',array('list_group'=>1)).$this->module->getFilterParams($fields_list,'mp_attribute');
        $paggination->limit =  10;
        $totalPages = ceil($totalRecords / $paggination->limit);
        if($page > $totalPages)
            $page = $totalPages;

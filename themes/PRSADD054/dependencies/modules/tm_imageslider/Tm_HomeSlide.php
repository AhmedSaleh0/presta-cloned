...
class Tm_HomeSlide extends ObjectModel
{
	public $title;
	public $description;
	public $url;
	...
	public static $definition = array(
		'table' => 'tmhomeslider_slides',
		'primary' => 'id_tmhomeslider_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'tmhomeslider` (`id_shop`, `id_tmhomeslider_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'tmhomeslider`
			WHERE `id_tmhomeslider_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_tmhomeslider_slides` = hs.`id_tmhomeslider_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_tmhomeslider_slides` as id_slide
			FROM `'._DB_PREFIX_.'tmhomeslider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'tmhomeslider` hs ON (hss.`id_tmhomeslider_slides` = hs.`id_tmhomeslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new Tm_HomeSlide($row['id_slide']);
